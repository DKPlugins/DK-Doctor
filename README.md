...
Proprietary. This is a **pre-release (beta) test build** — see [EULA.md](EULA.md) for the
beta End-User License Agreement. Using the build means accepting the EULA. The analyzer runs
entirely locally and does not upload your project files or telemetry. The desktop app can
optionally check GitHub for a newer release on launch (toggle in Settings); that check sends
no project data.
